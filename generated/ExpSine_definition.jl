...
"""
   ExpSine(; name, start_time, offset, amplitude, damping, frequency, phase)
...
"""
@component function ExpSine(; name, start_time=0, offset=0, amplitude=nothing, damping=nothing, frequency=nothing, phase=nothing)
  ...
  append!(__params, @parameters (damping::Float64 = damping))
  ...
  ### Equations
  __eqs = Equation[]
  push!(__eqs, y ~ ifelse(start_time < t, offset + amplitude * exp(-damping * (t - start_time)) * sin(2 * π * frequency * (t - start_time) + phase), offset))
...
Base.show(io::IO, a::MIME"image/svg+xml", t::typeof(ExpSine)) = print(io,
