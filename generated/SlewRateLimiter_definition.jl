...
"""
   SlewRateLimiter(; name, rising, falling, td)
...
| ------------ | ----------------------------------- | ------ | --------------- |
| `rising`         | Maximum rising slew rate                         | s-1  |    |
| `falling`         | Maximum falling slew rate                         | s-1  |   -rising |
| `td`         | Time derivative constant                         | s  |    |
...
"""
@component function SlewRateLimiter(; name, rising=nothing, falling=-rising, td=nothing)

  ### Symbolic Parameters
  __params = Any[]
  append!(__params, @parameters (rising::Float64 = rising), [description = "Maximum rising slew rate"])
  append!(__params, @parameters (falling::Float64 = falling), [description = "Maximum falling slew rate"])
  append!(__params, @parameters (td::Float64 = td), [description = "Time derivative constant"])
  ...
  ### Equations
  __eqs = Equation[]
  push!(__eqs, D(y) ~ clamp((u - y) / td, falling, rising))

  # Return completely constructed ODESystem
  return ODESystem(__eqs, t, __vars, __params; systems=__systems, defaults=__defaults, name, initialization_eqs=__initialization_eqs)
end
export SlewRateLimiter

Base.show(io::IO, a::MIME"image/svg+xml", t::typeof(SlewRateLimiter)) = print(io,
