...
"""
   Square(; name, start_time, offset, amplitude, frequency)
...
"""
@component function Square(; name, start_time=0, offset=0, amplitude=nothing, frequency=nothing)
  ...
  ### Equations
  __eqs = Equation[]
  push!(__eqs, y ~ ifelse(start_time < t, offset + (amplitude * (4 * floor(frequency * (t - start_time)) - 2 * floor(2 * (t - start_time) * frequency) + 1)), offset))
...
Base.show(io::IO, a::MIME"image/svg+xml", t::typeof(Square)) = print(io,
