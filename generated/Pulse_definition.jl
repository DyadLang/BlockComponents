...
"""
   Pulse(; name, start_time, offset, amplitude, duty_cycle, period)
...
"""
@component function Pulse(; name, start_time=0, offset=0, amplitude=nothing, duty_cycle=nothing, period=nothing)

  ### Symbolic Parameters
  __params = Any[]
  append!(__params, @parameters (start_time::Float64 = start_time))
  append!(__params, @parameters (offset::Float64 = offset))
  append!(__params, @parameters (amplitude::Float64 = amplitude))
  append!(__params, @parameters (duty_cycle::Float64 = duty_cycle))
  append!(__params, @parameters (period::Float64 = period))
  ...
  ### Equations
  __eqs = Equation[]
  push!(__eqs, y ~ ifelse(t > start_time, ifelse(t <= start_time + duty_cycle * period, amplitude, offset), offset))
...
Base.show(io::IO, a::MIME"image/svg+xml", t::typeof(Pulse)) = print(io,
