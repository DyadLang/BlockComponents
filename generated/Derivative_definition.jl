...
"""
@component function Derivative(; name, x0=0, T=nothing, k=1)

  ### Symbolic Parameters
  __params = Any[]
  append!(__params, @parameters (x0::Float64 = x0), [description = "Initial value of the derivative-filter state"])
  append!(__params, @parameters (T::Float64 = T), [description = "Time constant"])
  ...
  append!(__vars, @variables (x(t)), [description = "Derivative-filter state"])
  ...
  ### Equations
  __eqs = Equation[]
  push!(__eqs, D(x) ~ (u - x) / T)
  push!(__eqs, y ~ (k / T) * (u - x))
...
Base.show(io::IO, a::MIME"image/svg+xml", t::typeof(Derivative)) = print(io,
