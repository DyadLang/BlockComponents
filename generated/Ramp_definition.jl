...
"""
   Ramp(; name, start_time, offset, duration, height)
...
@component function Ramp(; name, start_time=0, offset=0, duration=nothing, height=nothing)
  ...
  append!(__params, @parameters (duration::Float64 = duration))
  append!(__params, @parameters (height::Float64 = height))
  ...
  ### Equations
  __eqs = Equation[]
  push!(__eqs, y ~ ifelse(start_time < t, ifelse(t < start_time + duration, offset + (t - start_time) * height / duration, offset + height), offset))
...
Base.show(io::IO, a::MIME"image/svg+xml", t::typeof(Ramp)) = print(io,
