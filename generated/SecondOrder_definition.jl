...
"""
@component function SecondOrder(; name, k=1, w=1, d=1)

  ### Symbolic Parameters
  __params = Any[]
  append!(__params, @parameters (k::Float64 = k), [description = "Gain"])
  append!(__params, @parameters (w::Float64 = w), [description = "Bandwidth (angular frequency)"])
  append!(__params, @parameters (d::Float64 = d), [description = "Relative damping"])

  ### Variables
  __vars = Any[]
  append!(__vars, @variables u(t), [input = true])
  append!(__vars, @variables y(t), [output = true])
  append!(__vars, @variables (x(t)), [description = "State of SecondOrder filter"])
  append!(__vars, @variables (xd(t)), [description = "Derivative state of SecondOrder filter"])
  ...
  __eqs = Equation[]
  push!(__eqs, D(x) ~ xd)
  push!(__eqs, D(xd) ~ w * (w * (k * u - x) - 2 * d * xd))
  push!(__eqs, y ~ x)
...
Base.show(io::IO, a::MIME"image/svg+xml", t::typeof(SecondOrder)) = print(io,
